...
local hitTargetThisSwing = false -- [[ NEW: Flag to prevent hitting multiple enemies ]]
...
local function processHit(otherPart)
    -- [[ FIX 1: SAFETY CHECK ]]
    -- Ensure the part and its parent still exist to prevent "Index nil" error
    if not otherPart or not otherPart.Parent then return end

    local character = otherPart.Parent
    local humanoid = character:FindFirstChild("Humanoid")
    
    -- [[ FIX 2: SINGLE TARGET LOGIC ]]
    -- If we already hit a zombie this swing, STOP.
    if hitTargetThisSwing then return end
    ...
    if humanoid then
        ...
        -- MARK AS HIT (So we don't hit this guy's other arm, or other zombies)
        hitTargetThisSwing = true

        -- 1. DEAL MAIN DAMAGE
        humanoid:TakeDamage(DAMAGE)
        
        -- 2. CLEAVE SKILL CHECK (Area of Effect)
        -- This ensures that even though the SWORD is single target, the SKILL is still AoE!
        if myPlayer then
            ...
            if cleaveLevel > 0 then
                ...
                overlapParams.FilterDescendantsInstances = {tool.Parent, character} -- Ignore self AND the guy we just hit
                ...
                for _, part in ipairs(parts) do
                    ...
                    if otherHum and otherHum.Health > 0 and not hitHumanoids[otherHum] then
                        ...
                        -- Add "Slash" particle for feedback
                        local emitter = Instance.new("ParticleEmitter")
                        emitter.Texture = "rbxassetid://243098098"
                        emitter.Lifetime = NumberRange.new(0.2); emitter.Rate = 0
                        emitter.Parent = vfx; emitter:Emit(5)
                    end
                end
            end
        end
...
tool.Activated:Connect(function()
    ...
    -- RESET THE HIT FLAG FOR THIS SWING
    hitTargetThisSwing = false
    ...
    local reachLevel = 0
    if myPlayer then
        reachLevel = getPlayerSkillLevel(myPlayer, "Reach") 
    end
    ...
    hitbox.CFrame = rootPart.CFrame * CFrame.new(0, 0, -hitboxSize.Z/2 - 2) 
